  ...
  return (
    <section className="w-full bg-black py-12 md:py-20 text-white overflow-hidden font-sans">
      ...
      <div className="w-full lg:w-[75vw] mx-auto px-6">
        ...
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto mb-8 md:mb-12 relative z-20">
          <div className="mb-4 flex justify-center">
            <span className="text-[#FF3A29] text-sm md:text-base font-medium uppercase tracking-wider font-poppins">
              Compare Features
            </span>
          </div>
          <h2 className="text-[32px] md:text-[56px] font-bold text-white tracking-tight leading-[1.1] font-poppins">
            See exactly what&apos;s included in every plan.
          </h2>
        </div>

        {/* Mobile scroll hint */}
        <p className="scroll-hint text-xs text-gray-500 text-right mb-2 pr-1">scroll →</p>

        <div className="overflow-x-auto -webkit-overflow-scrolling-touch rounded-2xl border border-white/10 bg-[#0A0A0A] no-scrollbar shadow-2xl">
          <table className="w-full text-left border-collapse min-w-[700px] font-poppins">
            <thead>
              <tr className="bg-white/[0.04]">
                <th className="sticky left-0 z-20 bg-[#0E0E0E] py-3 px-4 text-xs md:text-sm font-semibold text-gray-400 border-b border-r border-white/[0.06] w-[30%] shadow-[4px_0_12px_rgba(0,0,0,0.5)]">
                  Features
                </th>
                {['Starter', 'Next', 'Pro', 'Signature', "Founder's Circle"].map((plan) => (
                  <th key={plan} className="py-3 px-4 text-center text-xs md:text-sm font-semibold text-white border-b border-white/[0.06] w-[14%]">
                    {plan}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {displayFeatures.map((row, idx) => (
                <tr key={idx} className={`group hover:bg-[#111] transition-colors ${idx % 2 === 0 ? 'bg-white/[0.02]' : ''}`}>
                  <td className={`sticky left-0 z-10 bg-[#0A0A0A] group-hover:bg-[#111] py-2 px-4 text-xs md:text-sm border-b border-r border-white/[0.06] shadow-[4px_0_12px_rgba(0,0,0,0.5)] transition-colors ${row.isHeader ? 'font-semibold text-white pt-5' : 'text-gray-300'}`}>
                    {row.name}
                  </td>
                  {[row.starter, row.next, row.pro, row.signature, row.founder].map((val, i) => (
                    <td key={i} className={`py-2 px-4 text-center border-b border-white/[0.06] ${row.isHeader ? 'pt-5' : ''}`}>
                      {val ? (
              ...
              >
                <td className="sticky left-0 z-10 bg-[#0A0A0A] group-hover:bg-[#111] py-2 px-4 text-xs md:text-sm border-b border-r border-white/[0.06] shadow-[4px_0_12px_rgba(0,0,0,0.5)] transition-colors">
                  <div className="flex items-center gap-2 text-[#FF3A29] font-semibold">
                    {showAll ? 'View Less' : 'View All Features'}
                    <svg className={`w-3 h-3 transition-transform duration-300 ${showAll ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
                    </svg>
                  </div>
                </td>
                <td colSpan={5} className="py-2 px-4 border-b border-white/[0.06] bg-[#0A0A0A] group-hover:bg-[#111] transition-colors"></td>
              </tr>

              {/* Action Buttons Row */}
              <tr>
                <td className="sticky left-0 z-10 bg-[#0A0A0A] py-3 px-4 border-r border-white/[0.06] shadow-[4px_0_12px_rgba(0,0,0,0.5)]"></td>
                {['Starter', 'Next', 'Pro', 'Signature', "Founder's Circle"].map((plan, i) => (
                  <td key={plan} className="py-3 px-4 pt-6 pb-6">
                    <a
                      href="https://linkist.ai/choose-plan"
                      className={`flex items-center justify-center gap-1.5 w-full max-w-[140px] mx-auto py-2 px-3 rounded-lg font-semibold text-xs transition-all duration-300 no-underline ${
                        i === 2
                          ? 'bg-[#FF3A29] text-white shadow-[0_4px_14px_rgba(255,58,41,0.3)] hover:bg-[#e02e1a]'
